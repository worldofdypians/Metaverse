...
const EarnContent = ({
  ...
  const getTotalStakedNfts = async () => {
    try {
      ...
      const { wagmiClient } = await import("../../../../wagmiConnectors");

      const landBalance = await readContract(wagmiClient, {
        address: window.config.landnft_address,
        abi: window.LANDMINTING_ABI,
        functionName: "balanceOf",
        args: [window.config.nft_land_premiumstake_address],
        chainId: 1,
      });

      settotalStakesLandPremium(Number(landBalance));

      const cawsBalance = await readContract(wagmiClient, {
        address: window.config.nft_address,
        abi: window.CAWS_ABI,
        functionName: "balanceOf",
        args: [window.config.nft_caws_premiumstake_address],
        chainId: 1,
      });

      settotalStakesCawsPremium(Number(cawsBalance));
    } catch (error) {
