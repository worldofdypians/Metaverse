...
const TimepieceNFT = ({
  ...
  const [timepiecetraits, setTimepiecetraits] = useState([]);
  ...
  const sortNfts = useCallback(
    (sortValue) => {
      if (sortValue === "all") {
        ...
        setPriceCount((prev) => {
          const isAscending = prev % 2 === 0;
          const sorted = [...initialNfts].sort((a, b) => {
            return isAscending
              ? (a.priceUSD || 0) - (b.priceUSD || 0)
              : (b.priceUSD || 0) - (a.priceUSD || 0);
          });
          setTimepieceNFTS(sorted);
          return prev + 1;
        });
      } else if (sortValue === "lso") {
        const sorted = [...initialNfts]
          .sort((a, b) => b.isLatestSale - a.isLatestSale)
          .sort(
            (a, b) =>
  ...
  const getListedTimepiece = useCallback(async () => {
    const timepieceArray = [...timepiece, ...timepieceBought];
    const timepieceArray2 = [...timepiece];

    // Create a Map for faster lookups
    const timepieceBoughtMap = new Map(
      timepieceBought.map((item) => [item.tokenId, item])
    );
    ...
    if (uniqueTimepiece && uniqueTimepiece.length > 0) {
      let datedNfts = uniqueTimepiece
        .map((nft, index) => {
          const boughtItem = timepieceBoughtMap.get(nft.tokenId);
          const date = new Date((nft?.blockTimestamp || 0) * 1000);

          if (nft.tokenId == timepieceArray2[index]?.tokenId && !boughtItem) {
            return {
              ...nft,
              date: date,
              isListed: true,
            ...
            return {
              ...
              isLatestSale: true,
              type: "timepiece",
              LastSold: boughtItem.price,
              soldPriceType: boughtItem.payment_priceType,
            };
          } else if (
            nft.tokenId != timepieceArray2[index]?.tokenId &&
            nft?.buyer
  ...
  const fetchInitialTimepiece = useCallback(async () => {
    ...
    setLoading(false);
  }, [finalData, alltimepieceNfts]);

  const updateViewCount = useCallback(async (tokenId, nftAddress) => {
    try {
  ...
  const loadMore = useCallback(() => {
    setLoading(true);
    setNext((prev) => prev + nftsPerRow);
  }, [nftsPerRow]);

  const loadMore2 = useCallback(() => {
    setLoading(true);
    setNext2((prev) => prev + nftsPerRow2);
  }, [nftsPerRow2]);
  ...
  }, [count, next, next2, totalSupply, filterIds.length, loadMore, loadMore2]);
  ...
  useEffect(() => {
    ...
    const debouncedScroll = () => {
      ...
      timeoutId = setTimeout(onScroll, 100); // Debounce by 100ms
    };
  ...
  useEffect(() => {
    ...
    timepieceBought,
    nftCount,
    alltimepieceNfts.length,
    timepiece,
    getListedTimepiece,
  ]);

  // Handle appending new data when scrolling
  useEffect(() => {
    if (
      timepieceBought &&
  ...
  useEffect(() => {
    if (timepiecetraits && timepiecetraits.collection) {
  ...
  useEffect(() => {
    const loadData = async () => {
      const response = await fetch("/data/timepiecemetadata.json");
      const responseTraits = await fetch("/data/timepiecetraits.json");

      const data = await response.json();
      setTimepiecemetadata(data);

      const traitsData = await responseTraits.json();
      setTimepiecetraits(traitsData);
    };
