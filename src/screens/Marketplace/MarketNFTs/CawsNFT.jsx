...
const CawsNFT = ({
  ...
  const allCaws = 10000;
  const [cawsmetadata, setCawsmetadata] = useState([]);
  const [cawstraits, setCawstraits] = useState([]);
  ...
  const getListedCaws = useCallback(async () => {
    ...
    if (uniquecaws && uniquecaws.length > 0) {
      let datedNfts = uniquecaws
        .map((nft, index) => {
          const boughtItem = cawsBoughtMap.get(nft.tokenId);
          const date = new Date((nft?.blockTimestamp || 0) * 1000);

          if (nft.tokenId == cawsArray2[index]?.tokenId && !boughtItem) {
            return {
              ...nft,
              date: date,
              isListed: true,
          ...
          } else if (nft.tokenId == cawsArray2[index]?.tokenId && boughtItem) {
            return {
              ...nft,
              date: date,
              isListed: true,
              ...
              lastSoldTimeStamp: boughtItem.blockTimestamp,
  ...
  }, [count, next, next2, allCaws, filterIds.length, loadMore, loadMore2]);
  ...
  // Optimized scroll listener with cleanup and throttling
  useEffect(() => {
    let timeoutId = null;
    ...
    const debouncedScroll = () => {
      ...
      timeoutId = setTimeout(throttledScroll, 100); // Debounce by 100ms
    };
  ...
  useEffect(() => {
    if (cawstraits && cawstraits.collection) {
  ...
  }, [cawsBought, nftCount, allCawsNfts.length, caws, getListedCaws]);
  ...
  }, [finalData.length, cawsBought, allCawsNfts.length, fetchInitialCaws]);
  ...
  useEffect(() => {
    const loadData = async () => {
      const response = await fetch("/data/cawsmetadatas2.json");
      const responseTraits = await fetch("/data/cawstraits.json");
