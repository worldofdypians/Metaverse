...
const WoDNFT = ({
  ...
  const [landtraits, setLandtraits] = useState([]);
  ...
  const getListedWod = useCallback(async () => {
    ...
    if (uniquewod && uniquewod.length > 0) {
      let datedNfts = uniquewod
        .map((nft, index) => {
          const boughtItem = wodBoughtMap.get(nft.tokenId);
          const date = new Date((nft?.blockTimestamp || 0) * 1000);

          if (nft.tokenId == wodArray2[index]?.tokenId && !boughtItem) {
            return {
              ...nft,
              date: date,
              isListed: true,
          ...
          } else if (nft.tokenId == wodArray2[index]?.tokenId && boughtItem) {
            return {
              ...nft,
              date: date,
              isListed: true,
  ...
  }, [count, next, next2, allLandpiece, filterIds.length, loadMore, loadMore2]);
  ...
  }, [finalData.length, wodBought, allwodNfts.length, fetchInitialWod]);
  ...
  useEffect(() => {
    if (landtraits && landtraits.collection) {
  ...
  useEffect(() => {
    const loadData = async () => {
      const response = await fetch("/data/landmetadata.json");
      const responseTraits = await fetch("/data/landtraits.json");

      const data = await response.json();
      setLandmetadata(data);

      const traitsData = await responseTraits.json();
      setLandtraits(traitsData);
    };
