    ...
    >
      <div>
        <a href={`/nft/`}>
          {" "}
          <img
            style={{ width: single ? "400px" : "300px", padding: "10px" }}
            src="https://www.dypius.com/static/media/laptop2.221e066921ff71db7fc9.png"
            alt=""
          />
        </a>
      </div>
        ...
        <button
          ...
          onClick={() => {
            IsApprove === false
              ? approveNFT(nft, type)
              : !IsListed
              ? sell(nft, Price, PriceType, type)
              : cancelNFT(
                  NFtDetails.nftAddress,
                  NFtDetails.tokenId,
                  NFtDetails.payment_priceType
                );
          }}
        >
          {" "}
          {!IsApprove ? "Approve" : !IsListed ? "Sell it" : "Cancel List"}{" "}
        </button>
      </div>
    </div>
  );
