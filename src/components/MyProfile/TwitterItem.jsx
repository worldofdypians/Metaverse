...
const TwitterItem = ({ item, index, address, checkTwitter }) => {
  ...
  const checkAllTasks = async (tweetId) => {
    setGeneralLoading(true);
    console.log("works");

    const taskTypes = ["like", "comment", "retweet"];

    try {
      const results = await Promise.all(
        taskTypes.map((type) =>
          axios.post(`https://api.worldofdypians.com/twitter/verify-task`, {
      ...
      const allVerified = results.every((res) => res.data.verified);
      const anyFailed = results.some((res) => !res.data.verified);
  ...
  return (
    ...
    >
      ...
      <div className="d-flex flex-column align-items-center gap-2">
        <button
          className="check-task-btn px-2 py-1"
          onClick={() => checkAllTasks(item.tweetId)}
        >
          {generalLoading ? (
            <div
              className="spinner-border spinner-border-sm text-light"
