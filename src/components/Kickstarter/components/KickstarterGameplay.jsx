...
const KickstarterGameplay = () => {
  ...
  const gameImages = [
    ...
    {
      src: "https://cdn.worldofdypians.com/wod/kickstarterOpenWorld2.webp",
      alt: "Open World",
      //   icon: Star,
      rarity: "Open World",
      ...
      >
        ...
        <div className="custom-container mx-auto relative">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 mx-auto mb-20">
            {gameImages.slice(0, 4).map((game, index) => {
            ...
            {gameImages.slice(4, 8).map((game, index) => {
              // const IconComponent = game.icon;
              return (
                <div
                  key={index}
                  ...
                  onClick={() => {
                    setVideoPopup(true);
                    setVideoLink(game.id); // This is now the clean ID
                  }}
                >
                  {/* Glow effect behind each image */}
                  <div className="absolute inset-0 bg-gradient-to-r from-blue-400/30 via-cyan-500/30 to-purple-400/30 blur-2xl rounded-2xl  opacitytw-60 group-hover:opacitytw-100 transition-opacity duration-500"></div>
