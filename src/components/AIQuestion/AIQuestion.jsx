...
const AIQuestion = ({
  ...
  const handleUnlockQuestion = async () => {
    ...
    try {
      // Try using wagmi core (supports MetaMask, Injected, Binance Wallet, etc.)
      let txHash;
      try {
      ...
      if (txHash) {
        ...
        getAIQuestion(isBnb ? "bnb" : "opbnb", txHash);
      }
    } catch (e) {
      window.alertify?.error?.(e?.shortMessage || e?.message);
      setUnlockLoading(false);
      setUnlockStatus("error");
      console.error(e);
      setTimeout(() => setUnlockStatus("initial"), 3000);
    }
  };
