...
const NewDailyBonus = ({
  ...
  const handleOpBnbPool = async () => {
    await handleSwitchChainPool("0xcc", 204);
  };
  ...
  const handleTaikoPool = async () => {
    await handleSwitchChainPool("0x28c58", 167000, {
  ...
  const handleCorePool = async () => {
    await handleSwitchChainPool("0x45c", 1116, {
      supportsBinance: false,
    });
  };
  const handleVictionPool = async () => {
    await handleSwitchChainPool("0x58", 88, {
      supportsBinance: false,
    });
  };

  const handleSeiPool = async () => {
    await handleSwitchChainPool("0x531", 1329);
  };
  ...
  useEffect(() => {
    const found = chains.find((c) => c.chainId.includes(chainId));
    const foundIndex = chains.findIndex((c) => c.chainId.includes(chainId));
    if (found && foundIndex !== -1) {
      setChain(found.chainName);
                            ...
                            ? (() => {
                                ...
                                const specialStandardChestIds = new Set(
                                  uniqueRemainingStandardChests
                                    .filter((_, idx) => [1, 5, 7].includes(idx))
                                    .map((c) => c.chestId),
                                );
