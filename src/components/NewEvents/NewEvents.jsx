...
// All onchain interactions are via wagmi/viem now
import { styled, Tooltip, tooltipClasses } from "@mui/material";
import Countdown from "react-countdown";
import { useBinancePay } from "../../hooks/useBinancePay";
import "../../screens/Game/_game.scss";
...
const NewEvents = ({
  ...
  // Unified error handler for Beast Siege events
  const handleBeastEventError = (error, setters) => {
    const { setStatusColor, setStatus, setDepositState } = setters;
  ...
  const getBundlePrizesPuzzle = async () => {
    ...
    // } else {
    const result_puzzle = await readOnChain({
      address: puzzle_madness_address,
    ...
    if (result_puzzle) {
      setpuzzleMadnessWodAmount(Number(result_puzzle) / 1e18);
    }
    // }
  };

  const handleRefreshCountdownPuzzle = async () => {
    const purchaseTimestamp = await readOnChain({
      address: puzzle_madness_address,
      abi: PUZZLE_MADNESS_ABI,
    ...
    const purchaseTimestamp2 = await readOnChain({
      address: puzzle_madness2_address,
      abi: PUZZLE_MADNESS_ABI,
      functionName: "getTimeOfExpireBuff",
      args: [wallet],
    }).catch((e) => {
  ...
  const handleApprovalPuzzle = async (status) => {
    ...
    try {
      ...
      const spender =
        status === false ? puzzle_madness_address : puzzle_madness2_address;
  ...
  const getBundlePrizesDragon = async () => {
    const result_dragon_ruins = await readOnChain({
    ...
    if (result_dragon_ruins) {
      setDragonRuinsWodAmount(Number(result_dragon_ruins) / 1e18);
    }
  };
  ...
  const handleRefreshCountdownDragon = async () => {
    ...
    const purchaseDate = new Date(Number(purchaseTimestamp) * 1000); // Multiply by 1000 to convert to milliseconds
    const purchaseDate2 = new Date(Number(purchaseTimestamp2) * 1000); // Multiply by 1000 to convert to milliseconds
  ...
  const getBundlePrizesBear = async () => {
    const result_cold_bite = await readOnChain({
    ...
    if (result_cold_bite) {
      setColdBiteWodAmount(Number(result_cold_bite) / 1e18);
    }
  };
    ...
    try {
      const tokenToApprove =
        status === false
          ? window.config.wod_token_address
          : window.config.usdt_token_address;
      const spender = status === false ? coldBiteAddress : cold_bite2_address;
  ...
  const getBundlePrizesBeast = async () => {
    const result_fury_beast = await readOnChain({
    ...
    if (result_fury_beast) {
      setFuryBeastWodAmount(Number(result_fury_beast) / 1e18);
    }
  };
    ...
    try {
      const tokenToApprove =
        status === false
          ? window.config.wod_token_address
          : window.config.usdt_token_address;
      const spender = status === false ? furyBeastAddress : fury_beast2_address;
  ...
  const handleDepositBeast = async () => {
    ...
    try {
      ...
      handleBeastEventError(e, {
        setStatusColor,
        setStatus,
        setDepositState: setBeastDepositState,
      });
    }
  };
  ...
  const getBundlePrizesEagle = async () => {
    const result_wing_storm = await readOnChain({
    ...
    if (result_wing_storm) {
      setWingStormWodAmount(Number(result_wing_storm) / 1e18);
    }
  };
    ...
    try {
      const tokenToApprove =
        status === false
          ? window.config.wod_token_address
          : window.config.usdt_token_address;
      const spender = status === false ? wingStormAddress : wing_storm2_address;
  ...
  const handleDepositEagle = async () => {
    ...
    try {
      ...
      handleBeastEventError(e, {
        setStatusColor,
        setStatus,
        setDepositState: setEagleDepositState,
      });
    }
  };
  ...
  const getBundlePrizesScorpion = async () => {
    const result_scorpion_king = await readOnChain({
    ...
    if (result_scorpion_king) {
      setScorpionKingWodAmount(Number(result_scorpion_king) / 1e18);
    }
  };
  ...
  const handleRefreshCountdownScorpion = async () => {
    ...
    const purchaseTimestamp2 = await readOnChain({
      address: scorpion_king2_address,
      abi: SCORPION_KING_ABI,
      functionName: "getTimeOfDeposit",
      args: [coinbase],
    }).catch((e) => {
  ...
  const handleDepositScorpion = async () => {
    ...
    try {
      ...
      handleBeastEventError(e, {
        setStatusColor,
        setStatus,
        setDepositState: setScorpionDepositState,
      });
    }
  };
  ...
  const getBundlePrizesCyclops = async () => {
    const result_stone_eye = await readOnChain({
    ...
    if (result_stone_eye) {
      setStoneEyeWodAmount(Number(result_stone_eye) / 1e18);
    }
  };
    ...
    try {
      const tokenToApprove =
        status === false
          ? window.config.wod_token_address
          : window.config.usdt_token_address;
      const spender = status === false ? stoneEyeAddress : stone_eye2_address;
  ...
  const handleDepositCyclops = async () => {
    ...
    try {
      ...
      handleBeastEventError(e, {
        setStatusColor,
        setStatus,
        setDepositState: setCyclopsDepositState,
      });
    }
  };
  ...
  useEffect(() => {
    if (chainId || binancePay || window.WALLET_TYPE) {
      checkWalletAddr();
    }
  }, [chainId, binancePay, window.WALLET_TYPE]);
